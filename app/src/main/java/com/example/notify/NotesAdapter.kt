...
class NotesAdapter(private val context:Context?, private val notesList: RealmResults<Notes>)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        holder.itemView.titleTV.text = notesList(position).toString()
        holder.itemView.descTV.text = notesList(position).toString()
        holder.itemView.idTV.text = notesList(position).toString()
...
private operator fun <E> RealmResults<E>.invoke(position: Int): RealmResults<E> {
